...
class Grafo:
	...
	def Dijkstra(self, s):
		#destino = self.Nodos.getRandomNodo()
		t={}
		pq =[]
		S =[]
		pq.extend(self.Nodos.getNodos())
		
		inicial = self.Nodos.getNodo(s);
		inicial.setDistancia(0);
		self.priorizate(inicial,pq)

		while(len(pq) > 0):
			u_nodo = pq[0]
			if(u_nodo.getId() not in S):
				S.append(u_nodo.getId())

			for v in self.Aristas.getAristasById(u_nodo.getId()):
				if(v not in S):
					v_nodo = self.Nodos.getNodo(v)
					costo = self.Aristas.getCostoUV(u_nodo,v_nodo)
					if(costo == None):
						costo = self.Aristas.getCostoUV(v_nodo,u_nodo)

					if(v_nodo.getDistancia() > u_nodo.getDistancia() + costo):
						v_nodo.setDistancia(u_nodo.getDistancia() + costo)
						self.priorizate(v_nodo,pq)
						if(u_nodo.getId() not in t):
							t[u_nodo.getId()] = [v_nodo.getId()]
						else:
							#if(v_nodo.getId() not in t[u_nodo.getId()]):
							t[u_nodo.getId()].append(v_nodo.getId())

			pq.pop(0)
		return self.getGraphivzText(t,True,True)

	def priorizate(self, nodo, lista):	
		d1 = nodo.getDistancia()
		for index, node in enumerate(lista):
			d2 = node.getDistancia()
			if(d1 <= d2):
				lista.insert(index,nodo)
				print("Se agrego en " + str(index))
				break


	def getGraphivzText(self,grafo = None, dir = False, costo = False):
		header = "graph {\n"
		Nodos =""
		if(costo):
			for _id in self.Nodos.getIds():
				dist = "(" + str(self.Nodos.getNodo(_id).getDistancia())+")"
				Nodos += str(_id) + " [label = "+"\""+str(_id) +str(dist) +"\""+"]\n"
		else:
