...
class Aristas(Arista):
	...
	def printAristas(self):
		for arista in self.__Aristas.values():
			print(str(arista.getId()) +" "+ str(arista.getAristas()) + str(arista.getCostos()))
	...
	def getAristas(self,key):
		...
		if(key in self.__Aristas):
			aristas = self.__Aristas[key].getAristas()
			#if(len(aristas) == 0):
			#	aristas = self.getAristasById(key)
		return aristas
	...
	def getCostoUV(self, nodo_A, nodo_B):
		#print(str(nodo_A.getId()) + " " + str(nodo_B.getId()))
		if(not self.exists(nodo_A.getId()) 
			and self.exists(nodo_B.getId())):
			aux = nodo_A
			nodo_A = nodo_B
			nodo_B = aux

		for index, v in enumerate(self.__Aristas[nodo_A.getId()].getAristas()):
			if(nodo_B.getId() == v):
				return self.__Aristas[nodo_A.getId()].getCosto(index)